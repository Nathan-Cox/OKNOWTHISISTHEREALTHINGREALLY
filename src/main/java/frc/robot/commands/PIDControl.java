...
public class PIDControl {
	 ...
	 *            Small values prevents P and I terms from causing overshoot.
	 * @param f
	 *            Feed-forward gain. Open loop "best guess" for the output
	 *            should be. Only useful if setpoint represents a rate.
	 */
	 ...
	 * </list> A small D value can be useful for both improving response times,
	 * and preventing overshoot. However, in many systems a large D value will
	 * cause significant instability, particularly for large setpoint changes.
	 * <br>
	...
	public void setMaxIOutput(double maximum) {
		...
		// purposes.
		// The maxError is generated for simplifying math, since calculations
		// against
	...
	public double getOutput(double actual, double setpoint) {
		...
		// Calculate F output. Notice, this depends only on the setpoint, and
		// not the error.
		Foutput = F * setpoint;
		...
		// For last output, we can assume it's the current time-independent
		// outputs.
		if (firstRun) {
			lastActual = actual;
		...
		// Calculate D Term
		// Note, this is negative. This actually "slows" the system if it's
		// doing
		...
		// The Iterm is more complex. There's several things to factor in to
		// make it easier to deal with.
		// 1. maxIoutput restricts the amount of output contributed by the
		// Iterm.
		...
		if (minOutput != maxOutput && !bounded(output, minOutput, maxOutput)) {
			...
			// Setting to current error ensures a smooth transition when the P
			// term
			...
			// In addition to output limiting directly, we also want to prevent
			// I term
